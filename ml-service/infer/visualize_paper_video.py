...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Visualize paper-mode C3D + Multi-SVM predictions with boxes and zone risk overlays."
    )
    ...
    parser.add_argument("--window-name", default="SafeFlow Paper Visualizer")
...
def main() -> None:
    ...
    try:
        for clip_idx, (frames, frame_ts_ms) in enumerate(_iter_video_clips(Path(args.video), int(args.clip_len))):
            ...
            global_risk = float(clamp01(risk_from_behavior_label(pred_label, 0.5)))

            resized_bgr, _gray = preprocess_frame(rep, width=width, height=height)
            detections = []
            ...
            for z in zones:
                ...
                current.append(float(clamp01(r)))
            smoothed, ewma_state = apply_ewma(zone_ids, np.asarray(current, dtype=np.float32), ewma_state, 0.7)
            anomaly_by_zone = {zone_ids[i]: float(smoothed[i]) for i in range(len(zone_ids))}
                ...
                zones_payload.append(
                    {
                        ...
                        "anomaly": float(clamp01(anomaly_by_zone[z.zone_id])),
            ...
            top = f"clip:{clip_idx} behavior:{pred_label} p:{pred_conf:.2f} risk:{global_risk:.2f} dets:{len(detections)}"
