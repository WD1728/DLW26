...
parser.add_argument('gpu',metavar='GPU', type=str, nargs='?', default='0',
                    help='GPU id to use (default: 0).')

parser.add_argument('task',metavar='TASK', type=str, nargs='?', default='csrnet_',
                    help='task id prefix used for checkpoints (default: csrnet_).')
...
def validate(val_list, model, criterion, device):
    ...
    with torch.no_grad():
        for i,(img, target) in enumerate(test_loader):
            img = img.to(device, non_blocking=True)
            output = model(img)
            mae += abs(output.detach().sum().item() - target.sum().item())
